#!/usr/bin/env python3
"""ROS 2 action server that describes a visible person using camera0 and the VLM service."""
...
class DescribeHumanBlackboard:
    request: VlmQuery.Request | None = None
    attempt_count: int = 0
    max_retries: int = 2
    last_error: str = ""
    last_reason: str = ""
...
class DescribeHumanActionNode(Node):
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            ...
            f"vlm_service={self.vlm_query_service} | speak_action={self.speak_action_name} | "
            f"default_camera={self.default_camera_name} | max_retries={self.max_retries} | "
    ...
    def _create_tree(self) -> py_trees.behaviour.Behaviour:
        return py_trees.composites.Sequence(
            ...
            memory=False,
            children=[
                BlackboardAction("QueryVlm", self.blackboard, self, self._bt_query_vlm),
    ...
    def _build_request(self) -> VlmQuery.Request:
        request = VlmQuery.Request()
        request.need_image = True
        request.camera_name = self.default_camera_name
        request.prompt = HUMAN_DESCRIPTION_PROMPT
        request.reasoning_mode = "thinking" if self.force_thinking else "fast"
        request.user_input = (
    ...
    def _query_vlm(self, request: VlmQuery.Request) -> Any:
        if not self._vlm_client.wait_for_service(timeout_sec=self.service_wait_timeout_sec):
            raise RuntimeError(f"VLM query service '{self.vlm_query_service}' is not available.")
        future = self._vlm_client.call_async(request)
        ok, response, error_text = self._wait_for_future(future, self.request_timeout_sec)
        if not ok:
            raise RuntimeError(f"VLM request failed: {error_text}")
        if response is None:
            raise RuntimeError("VLM service returned no response.")
        if not response.success:
            raise RuntimeError(str(response.message) or "VLM service request failed.")
        return response
    ...
    def _parse_data_text(self, data_text: str) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        if not isinstance(parsed, dict):
            raise RuntimeError("VLM data_text is not a JSON object.")
        entities = parsed.get("entities", {})
        if not isinstance(entities, dict):
            raise RuntimeError("VLM data_text.entities is not a JSON object.")
    ...
    def _bt_query_vlm(self, blackboard: DescribeHumanBlackboard, node: "DescribeHumanActionNode") -> Status:
        ...
        blackboard.last_status = (
            f"Attempt {blackboard.attempt_count}: querying VLM with reasoning_mode="
            f"{blackboard.request.reasoning_mode}"
        )
        try:
