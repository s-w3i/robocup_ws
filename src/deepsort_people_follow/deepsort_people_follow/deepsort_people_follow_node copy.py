...
class DeepSortPeopleFollowNode(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("model_path", "/home/usern/robocup_ws/yolo11n.pt")
        ...
        self.declare_parameter("max_age", 75)
        self.declare_parameter("n_init", 2)
        self.declare_parameter("max_cosine_distance", 0.20)
        self.declare_parameter("nn_budget", 150)
        ...
        self.declare_parameter("publish_rate_hz", 18.0)
        self.declare_parameter("diagnostics_period_s", 1.0)
        self.declare_parameter("max_input_color_rate_hz", 24.0)
        self.declare_parameter("max_input_depth_rate_hz", 18.0)
        ...
        self.declare_parameter("lost_hold_sec", 1.0)
        self.declare_parameter("reacquire_max_dist_m", 2.2)
        self.declare_parameter("reacquire_max_vel_mps", 1.0)
        self.declare_parameter("motion_weight", 0.15)
        ...
        self.max_input_color_rate_hz = max(0.0, float(self.get_parameter("max_input_color_rate_hz").value))
        self.max_input_depth_rate_hz = max(0.0, float(self.get_parameter("max_input_depth_rate_hz").value))
        ...
        self._last_color_accept_s = 0.0
        self._last_depth_accept_s = 0.0
        ...
        self.get_logger().info(f"Lazy subscriptions: {str(self.lazy_subscriptions).lower()}")
        self.get_logger().info(
            "Input throttles: "
            f"color={self.max_input_color_rate_hz:.2f} Hz, "
            f"depth={self.max_input_depth_rate_hz:.2f} Hz"
        )
    ...
    def _subscribe_input_streams(self) -> None:
        ...
        self.get_logger().info("Subscribed to DeepSORT input streams.")
    ...
    def _unsubscribe_input_streams(self) -> None:
        ...
        self.get_logger().info("Unsubscribed from DeepSORT input streams.")
    ...
    def _init_backends(self) -> None:
        ...
        try:
            ...
            tracker, tracker_uses_internal_embedder, tracker_embedder_label = self._build_tracker_instance()
            self._set_tracker_instance(
                tracker=tracker,
                tracker_uses_internal_embedder=tracker_uses_internal_embedder,
                tracker_embedder_label=tracker_embedder_label,
            )
            ...
            self._set_tracker_instance(
                tracker=None,
                tracker_uses_internal_embedder=False,
                tracker_embedder_label="unavailable",
            )
    ...
    def _set_tracker_instance(
        self,
        tracker: Any,
        tracker_uses_internal_embedder: bool,
        tracker_embedder_label: str,
    ) -> None:
        with self._tracker_lock:
            self._tracker = tracker
            self._tracker_uses_internal_embedder = tracker_uses_internal_embedder
            self._tracker_embedder_label = tracker_embedder_label

    def _snapshot_tracker_state(self) -> tuple[Any, bool, str]:
        with self._tracker_lock:
            return (
                self._tracker,
                self._tracker_uses_internal_embedder,
                self._tracker_embedder_label,
            )

    def _build_tracker_instance(self) -> tuple[Any, bool, str]:
        if self._DeepSort_cls is None:
        ...
        tracker_uses_internal_embedder = False
        tracker_embedder_label = "hsv_histogram"
        ...
        if backend == "torchreid":
            try:
                ...
                tracker_uses_internal_embedder = True
                if self.embedder_weights_path:
                    model_file = pathlib.Path(self.embedder_weights_path).name
                    tracker_embedder_label = (
                        f"torchreid:{self.torchreid_model_name} ({model_file})"
                    )
                else:
                    tracker_embedder_label = f"torchreid:{self.torchreid_model_name}"
                return tracker, tracker_uses_internal_embedder, tracker_embedder_label
            except Exception as exc:
            ...
            try:
                ...
                tracker = self._DeepSort_cls(**common_kwargs, **embedder_kwargs)
                tracker_uses_internal_embedder = True
                tracker_embedder_label = embedder_name
                return tracker, tracker_uses_internal_embedder, tracker_embedder_label
            except Exception as exc:
                if not self.fallback_to_hsv_embedder:
                    raise
            ...
            try:
                ...
                tracker_uses_internal_embedder = True
                tracker_embedder_label = "mobilenetv2"
                return tracker, tracker_uses_internal_embedder, tracker_embedder_label
            except Exception as exc:
                if not self.fallback_to_hsv_embedder:
                    raise
        ...
        tracker = self._DeepSort_cls(**common_kwargs, embedder=None)
        return tracker, tracker_uses_internal_embedder, tracker_embedder_label
    ...
    def _on_color_image(self, msg: Image) -> None:
        ...
        if self.max_input_color_rate_hz > 0.0:
            min_period_s = 1.0 / self.max_input_color_rate_hz
            if now_s - self._last_color_accept_s < min_period_s:
                return
        ...
        with self._data_lock:
            ...
            self._latest_color_seq += 1
            self._last_color_accept_s = now_s

    def _on_depth_image(self, msg: Image) -> None:
        now_s = time.monotonic()
        if self.max_input_depth_rate_hz > 0.0:
            min_period_s = 1.0 / self.max_input_depth_rate_hz
            if now_s - self._last_depth_accept_s < min_period_s:
                return
        ...
        with self._data_lock:
            ...
            self._last_depth_accept_s = now_s
    ...
    def _handle_tracking_service(
        ...
        if request.enable:
            tracker, _, _ = self._snapshot_tracker_state()
            if self._detector is None or tracker is None:
                self._init_backends()
                tracker, _, _ = self._snapshot_tracker_state()
            if self._detector is None or tracker is None:
    ...
    def _process_frame(self, frame: np.ndarray, frame_stamp: Any, frame_id: str) -> None:
        tracker, tracker_uses_internal_embedder, _ = self._snapshot_tracker_state()
        if self._detector is None or tracker is None:
        ...
        if results:
            ...
            if boxes is not None:
                for box in boxes:
                    ...
                    if not tracker_uses_internal_embedder:
                        crop = frame[by : by + bh, bx : bx + bw]
                        embeddings.append(self._compute_embedding(crop))

        if tracker_uses_internal_embedder:
            ...
            tracks = tracker.update_tracks(raw_detections, embeds=embeddings)
    ...
    def _update_follow_target(
        ...
        if self._follow_track_id is not None:
            ...
            if current is not None:
                ...
                published = True
            elif self._last_follow_pose is not None and (now_s - self._last_follow_seen_s) <= self.lost_hold_sec:
        ...
        if not published and self._follow_track_id is None and self._last_follow_pose is not None:
