...
def build_guest1_to_guest2_intro_text(blackboard: Blackboard) -> str:
    ...
    intro_details = build_guest_intro_detail_clauses(guest1)
    ...
    if intro_details and appearance_description:
        return f"{guest2_name}, this is {guest1_name}, {intro_details}, and is {appearance_description}."
    if intro_details:
        return f"{guest2_name}, this is {guest1_name}, {intro_details}."
    if appearance_description:
        return f"{guest2_name}, this is {guest1_name}, and is {appearance_description}."
    return f"{guest2_name}, this is {guest1_name}."


def has_no_food_allergy_text(value: str) -> bool:
    return value.strip().lower() == "no food allergy"


def has_no_food_preference_text(value: str) -> bool:
    return value.strip().lower() == "no particular preference"


def build_guest_intro_detail_clauses(guest: dict[str, Any]) -> str:
    drink = str(guest.get("drink", "")).strip()
    food_flavour_preference = str(guest.get("food_flavour_preference", "")).strip()
    food_allergy = str(guest.get("food_allergy", "")).strip()

    clauses: list[str] = []
    if drink:
        clauses.append(f"whose favourite drink is {drink}")
    if food_flavour_preference:
        if has_no_food_preference_text(food_flavour_preference):
            clauses.append("who has no particular food flavour preference")
        else:
            clauses.append(f"whose food flavour preference is {food_flavour_preference}")
    if food_allergy:
        if has_no_food_allergy_text(food_allergy):
            clauses.append("who has no food allergy")
        else:
            clauses.append(f"whose food allergy is {food_allergy}")

    return join_spoken_list(clauses)
...
class NavigateAndPrepareIntroState(State):
    def __init__(self, prompt_factory, result_field: str, destination_key_field: str) -> None:
        ...
        self._vlm_client = self._node.create_client(VlmQuery, self._service_name)
    ...
    def execute(self, blackboard: Blackboard) -> str:
        ...
        if not self._vlm_client.wait_for_service(timeout_sec=5.0):
        ...
        request.timeout_sec_override = float(blackboard["vlm_intro_timeout_sec"])
        request.user_input = ""
        ...
        while time.monotonic() < query_wait_deadline:
            if future.done():
                try:
                    ...
                    blackboard["last_error"] = f"VLM query future failed: {exc}"
                    return ABORT
                if response is None:
...
class StoreAskNameAndDrinkState(ActionState):
    ...
    def result_handler(self, blackboard: Blackboard, result: AskNameAndDrink.Result) -> str:
        ...
        guest["food_flavour_preference"] = result.food_flavour_preference.strip()
        guest["food_allergy"] = result.food_allergy.strip()
...
def host_intro_prompt(blackboard: Blackboard) -> str:
    ...
    return (
        ...
        "Set entities to an object with keys guest_name, favourite_drink, food_flavour_preference, and food_allergy. "
        "Include the guest name, favourite drink, food flavour preference, and food allergy in the spoken sentence. "
        "If the food allergy is 'no food allergy', say that the guest has no food allergy. "
        "If the food flavour preference is 'no particular preference', say that the guest has no particular food flavour preference. "
        f"Host name: {HOST}. "
        f"Guest name: {guest['name']}. "
        f"Favourite drink: {guest['drink']}. "
        f"Food flavour preference: {guest['food_flavour_preference']}. "
        f"Food allergy: {guest['food_allergy']}. "
...
def build_state_machine() -> StateMachine:
    ...
    sm.add_state(
        ...
        transitions={NAVIGATION_DONE: "ANNOUNCE_READY_FOR_NEW_GUEST_AT_START", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
        "ANNOUNCE_READY_FOR_NEW_GUEST_AT_START",
        SpeakState(lambda _: "I am ready to accept new guest", "spoken"),
        transitions={"spoken": "STARTUP_DELAY", ABORT: ABORT},
    )
    sm.add_state(
        "STARTUP_DELAY",
    ...
    sm.add_state(
        ...
        transitions={NAVIGATION_DONE: "GREET_GUEST1", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
        "GREET_GUEST1",
    ...
    sm.add_state(
        ...
        transitions={"spoken": "ENSURE_CHAIR2_BEFORE_EMPTY_CHAIR_DETECTION_GUEST1", ABORT: ABORT},
    )
    sm.add_state(
        "ENSURE_CHAIR2_BEFORE_EMPTY_CHAIR_DETECTION_GUEST1",
        EnsureChair2NavigationState(),
        transitions={NAVIGATION_DONE: "ANNOUNCE_EMPTY_CHAIR_DETECTION_GUEST1", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
    ...
    sm.add_state(
        ...
        transitions={"spoken": "NAVIGATE_BACK_TO_START", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
    ...
    sm.add_state(
        ...
        NavigateToInitialPoseState(),
        transitions={NAVIGATION_DONE: "ANNOUNCE_READY_FOR_NEW_GUEST_AFTER_GUEST1", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
        "ANNOUNCE_READY_FOR_NEW_GUEST_AFTER_GUEST1",
        SpeakState(lambda _: "I am ready to accept new guest", "spoken"),
        transitions={"spoken": "PREPARE_GUEST2", ABORT: ABORT},
    )
    sm.add_state(
        "PREPARE_GUEST2",
    ...
    sm.add_state(
        ...
        NavigateToDetectedGuestState(),
        transitions={NAVIGATION_DONE: "GREET_GUEST2", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
        "GREET_GUEST2",
        SpeakState(lambda _: f"Hello, Welcome to {HOST} house", "spoken"),
    ...
    sm.add_state(
        ...
        transitions={"spoken": "ENSURE_CHAIR2_BEFORE_EMPTY_CHAIR_DETECTION_GUEST2", ABORT: ABORT},
    )
    sm.add_state(
        "ENSURE_CHAIR2_BEFORE_EMPTY_CHAIR_DETECTION_GUEST2",
        EnsureChair2NavigationState(),
        transitions={NAVIGATION_DONE: "ANNOUNCE_EMPTY_CHAIR_DETECTION_GUEST2", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
    ...
    sm.add_state(
        ...
        AnnounceEmptyChairFoundState(),
        transitions={"spoken": "NAVIGATE_TO_INITIAL_POSE_AFTER_GUEST2", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
        "NAVIGATE_TO_INITIAL_POSE_AFTER_GUEST2",
        NavigateToInitialPoseState(),
        transitions={NAVIGATION_DONE: "ANNOUNCE_TASK_COMPLETED", ABORT: ABORT, TIMEOUT: ABORT},
    )
    sm.add_state(
