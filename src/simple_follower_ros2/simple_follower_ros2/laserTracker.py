...
class LaserTracker(Node):
	...
	def __init__(self):
		...
		self.vision_laser_angle_tolerance = self.declare_parameter('vision_laser_angle_tolerance', 0.45).value
		self.vision_laser_distance_tolerance = self.declare_parameter('vision_laser_distance_tolerance', 0.8).value
		...
		self.last_vision_person_angle = None
		self.last_vision_person_distance = None
	...
	def handleVisionResponse(self, future):
		...
		if not response.success:
			self.publishInfo(
				f'vision:person not detected ({response.message})',
				warn=True,
			)
			return

		person_seen = False
		for class_name, confidence, pose in zip(
			...
			if 'person' in class_name.lower() or 'human' in class_name.lower():
				person_seen = True
				person_x = float(pose.pose.position.x)
				person_y = float(pose.pose.position.y)
				self.last_vision_person_angle = float(np.arctan2(person_y, person_x))
				self.last_vision_person_distance = float(np.hypot(person_x, person_y))
				break
		...
		if person_seen:
			...
			self.publishInfo(
				'vision:person not detected '
				f'(no person class >= {self.vision_min_confidence:.2f})',
				warn=True,
			)

	def visionRecentlySawPerson(self):
		if not self.vision_validation_enabled or not self.require_vision_for_acquire:
			return True, 'vision gating disabled'
		if self.last_vision_person_time is None:
			return False, 'no recent camera person detection'
		elapsed = (self.get_clock().now() - self.last_vision_person_time).nanoseconds / 1e9
		if elapsed <= float(self.vision_valid_timeout):
			return True, f'last valid detection {elapsed:.1f}s ago'
		return False, (
			f'last valid detection expired {elapsed:.1f}s ago '
			f'(timeout={float(self.vision_valid_timeout):.1f}s)'
		)
	...
	def angleDifference(self, first_angle, second_angle):
		return float(np.arctan2(
			np.sin(first_angle - second_angle),
			np.cos(first_angle - second_angle),
		))

	def selectAcquireTarget(self, valid_clusters):
		if (
			self.vision_validation_enabled
			and self.require_vision_for_acquire
			and self.last_vision_person_angle is not None
			and self.last_vision_person_distance is not None
		):
			matched_clusters = []
			for cluster in valid_clusters:
				cluster_angle = float(np.arctan2(cluster['centroid_y'], cluster['centroid_x']))
				angle_error = abs(self.angleDifference(cluster_angle, self.last_vision_person_angle))
				distance_error = abs(cluster['distance'] - self.last_vision_person_distance)
				if (
					angle_error <= float(self.vision_laser_angle_tolerance)
					and distance_error <= float(self.vision_laser_distance_tolerance)
				):
					matched_clusters.append((angle_error, distance_error, cluster))

			if not matched_clusters:
				self.publishInfo(
					'laser:no laser cluster matches camera-confirmed person; ignoring nearer objects',
					warn=True,
				)
				return None

			_, _, target = min(matched_clusters, key=lambda item: (item[0], item[1]))
			return target

		return min(valid_clusters, key=lambda cluster: cluster['distance'])

	def acquireTarget(self, scan_data, points):
		vision_valid, vision_reason = self.visionRecentlySawPerson()
		if not vision_valid:
			self.publishInfo(
				f'laser:person not detected, waiting for camera confirmation '
				f'({vision_reason})',
				warn=True,
			)
			return False
		...
		target = self.selectAcquireTarget(valid_clusters)
		if target is None:
			return False
